...
For human-specific metadata (disease staging, comorbidities, treatment history), see link:../templates/human/README.adoc[Human Template].
...
The `characteristics[tissue]` column should not be used as a replacement for `characteristics[organism part]`. When tissue-related columns are needed, use them alongside `characteristics[organism part]` for additional hierarchy or provenance, for example `characteristics[tissue supergroup]` for broader grouping and `characteristics[sampling site]` for finer source context.
====
...
For additional columns, see the link:sdrf-terms.tsv[SDRF Terms Reference] and the link:../templates/human/README.adoc[Human Template]. 
