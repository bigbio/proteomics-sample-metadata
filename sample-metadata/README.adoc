...
in the `characteristics[individual]` column. Non-PDX samples shouild contain the `not applicable` value in the `Characteristics[xenograft]` and the
`characteristics[individual]` column.
