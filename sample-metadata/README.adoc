...
Technical and biological replicates are important to understand the experimental design. Multiple definitions can be attributed to technical and biological replicates and it will depends on the researcher, annotator to define and consider what in the data analysis is each concept. A good starting point for the SDRF specification is the following:
...
In MS-based proteomics a technical can be, for example, doing the full sample preparation from extraction to mass spectrometry multi times to control variability in the instrument, or sample preparation. Another valid example is run run repeat, replicate only one part of the analytical method, for example, run the sample twice on the LC-MS/MS. Technical replicates indicate if your measurements are scientifically robust or noisy, and how large the measured effect must be to stand out above that noise.
...
NOTE: The comment[technical replicate] column is OPTIONAL and can be easily guess from the data structure if the column `comment[technical replicate]` is not provided. If the same sample has the fraction identifier repeated it can be interpreted as technical replicate. However, some submitters can make more clear that annotation.

**Biological replicate**: parallel measurements of biologically distinct samples that capture random biological variation, which may itself be a subject of study or a source of noise. Biological replicates address if and how widely the results of an experiment can be generalized. For example, repeating a particular assay with independently generated samples, individuals or samples derived from various cell types, tissue types, or organisms, to see if similar results can be observed. Context is critical, and appropriate biological replicates will indicate whether an experimental effect is sustainable under a different set of biological variables or an anomaly itself.

NOTE: In SDRF proteomics biological replicates can be annotated using `characteristics[biological replicate]` but is OPTIONAL. This information can be also guess using the `factor value`
and the `source name`, if two different source names (samples) are provided for the same factor value (condition) in the experiment they can be considered as biological replicates.
...
TIP: Multiple tools like MaxQuant or MSstats recognize some of this technical and biological replicates as Groups. In MSstats for example: Technical replicate correspond to `TechRepMixture`, while biological replicates correspond to `BioReplicate`.
