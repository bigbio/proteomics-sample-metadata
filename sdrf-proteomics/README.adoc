...
|`comment[sdrf template]` |Template name and version used for annotation. Two formats are supported: key=value format (`NT=name;version=vX.Y.Z`) or simple format (`name vX.Y.Z`). Multiple templates can be specified using multiple columns. |NT=human;version=v1.1.0 or human v1.1.0 |OPTIONAL |PRIDE:0000832
...
- Simple format: `template_name vX.Y.Z`
...
|Human MS proteomics |`comment[sdrf template]` = `NT=human;version=v1.1.0` or `human v1.1.0`
|Mouse MS proteomics |`comment[sdrf template]` = `NT=vertebrates;version=v1.1.0` or `vertebrates v1.1.0`
