...
<td>Template(s) used, comma-separated. Only list leaf templates; parent templates are implied by inheritance (e.g., crosslinking implies ms-proteomics)</td>
<td>human,crosslinking</td>
...
<td>Version(s) of the template(s). Use single value if all templates share the same version; comma-separated only when versions differ</td>
<td>v1.1.0</td>
<td><span class="optional-badge">OPTIONAL</span></td>
...
|`template` |Template(s) used, comma-separated. Only list leaf templates; parent templates are implied by inheritance |human,crosslinking |OPTIONAL |PRIDE:0000832
|`template_version` |Version(s) of the template(s). Use single value if all same; comma-separated only when versions differ |v1.1.0 |OPTIONAL |PRIDE:0000833
...
- `crosslinking` inherits from `ms-proteomics` → use `#template=human,crosslinking` (not `ms-proteomics,human,crosslinking`)
- `olink` inherits from `affinity-proteomics` → use `#template=human,olink` (not `affinity-proteomics,olink,human`)

**Version simplification:**

- If all templates share the same version, use a single value: `#template_version=v1.1.0`
...
|link:templates/base/README.adoc[base]
...
TIP: The **base** template is an internal construction artifact that defines shared columns inherited by all other templates. It should never be used directly by users. This specification document itself serves as the documentation for base template columns - all columns defined in the <<from-sample-metadata,sample metadata>> and <<from-sample-data,data file metadata>> sections are inherited by technology and sample templates.
...
|Human Olink
|human + olink
|`#template=human,olink`

|Human DIA with cell lines
|human + dia-acquisition + cell-lines
|`#template=human,dia-acquisition,cell-lines`
|===
...
This section describes templates that are actively under discussion. If you want to discuss a particular template, please create an https://github.com/bigbio/proteomics-metadata-standard/issues[issue on GitHub].

=== MS-Imaging Datasets

Discussions about MS-Imaging representation can be found in https://github.com/bigbio/proteomics-metadata-standard/issues/451[issue #451]. Key considerations for MSI datasets:

**Metadata that may not apply to MSI:**

- **Reduction and Alkylation**: For most MSI studies this would be "not applicable"
- **Fraction identifier**: Never used in MSI - use "not applicable"
- **MS2/Database search terms**: Not mandatory, can be ignored for MSI data

**MSI-specific considerations:**

- **File type**: imzML is a composite format (imzML + ibd files). Current practice references only the imzML file.
- **Multiple samples per run**: Multiple tissues can be placed on one slide. Samples are distinguished by position, requiring additional position identifiers or coordinates.

**Potential additional MSI metadata columns:**

- **Origin of cleavage agent**: Important for calibration/QC (bovine vs. porcine trypsin)
- **Matrix**: For MALDI experiments, matrix composition helps with peak removal
- **Ionization source**: Customized sources may not match standard instrument terms
- **Analyte**: MSI can measure phospholipids, N-glycans, and other analytes beyond peptides

=== Derived Samples (Patient-Derived Xenografts)

A prototype for derived samples is documented in the <<derived-samples>> section. Outstanding issues include:

- Full examples with multiple species and mixture types are needed
- Capturing relationships between samples (e.g., in xenografts, how to link the human tumor sample to the mouse host)

=== Turnover and TMT Datasets

Turnover experiments may contain multiple labels - for example, TMT for quantification combined with SILAC for turnover/differential expression. See https://github.com/bigbio/proteomics-metadata-standard/tree/master/annotated-projects/PXD017710[PXD017710] for an example that was originally annotated as two independent datasets.
