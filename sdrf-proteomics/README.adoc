...
However, the information regarding the sample and its relation to the data files (**Figure 1**) is mostly missing in public repositories <<ref1>>.
...
[[quick-start]]
== Understanding SDRF-Proteomics

The SDRF-Proteomics file format describes the sample characteristics and the relationships between samples and data files. The format is a tab-delimited file where each *ROW* corresponds to a relationship between a Sample and a Data file, each *COLUMN* corresponds to an attribute/property of the Sample, the Data file, or the Factor values, and the value in each **CELL** is the specific value of the property for a given Sample/Data file/Factor value.

The SDRF-Proteomics format contains three main sections:

- The first section contains the <<from-sample-metadata,sample metadata>> using `characteristics[...]` columns (e.g., organism, disease)
- The second section contains the <<from-sample-data,data file metadata>> using `comment[...]` columns (e.g., instrument, label)
- The third section contains the <<study-variables,factor values>> using `factor value[...]` columns to indicate variables under study

Each row links one sample to one data file. If you're new to SDRF-Proteomics, here's a minimal example to get you started:
...
<td class="data-col">1</td>
<td class="data-col">1</td>
<td class="data-col">sample_3.raw</td>
<td class="factor-col">not available</td>
</tr>
</tbody>
</table>
...
|===
_Blue: Sample metadata | Green: Data file metadata | Orange: Factor values. Note: sample_3 demonstrates use of "not available" for unknown values._
endif::[]

NOTE: This minimal example shows a **mass spectrometry-based** proteomics experiment. For detailed requirements see the link:templates/ms-proteomics/README.adoc[MS-Proteomics template]. For **affinity-based proteomics** (Olink, SomaScan), different columns are required - see the link:templates/affinity-proteomics/README.adoc[Affinity-Proteomics template].
...
For a step-by-step tutorial on creating your first SDRF file, see the link:quickstart.adoc[Quick Start Guide].
...
- **Core templates** (organism-based): link:templates/human/README.adoc[human], link:templates/vertebrates/README.adoc[vertebrates], link:templates/invertebrates/README.adoc[invertebrates], link:templates/plants/README.adoc[plants]
- **Specialized templates** (experiment-type): link:templates/cell-lines/README.adoc[cell-lines] (sample), link:templates/dda-acquisition/README.adoc[DDA acquisition] (data), link:templates/dia-acquisition/README.adoc[DIA acquisition] (data), link:templates/single-cell/README.adoc[single-cell] (mixed), link:templates/affinity-proteomics/README.adoc[affinity-proteomics] (data), link:templates/crosslinking/README.adoc[crosslinking] (data), link:templates/immunopeptidomics/README.adoc[immunopeptidomics] (mixed), link:templates/metaproteomics/README.adoc[metaproteomics] (mixed)
...
== SDRF-Proteomics Detailed Specification

This section provides the detailed rules and requirements for the SDRF-Proteomics format.
...
<td>Template declaration with version. Use one <code>#template</code> line per template. Format: <code>template_name,version=vX.Y.Z</code></td>
<td>human,version=v1.1.0</td>
...
<td>Tool or origin that generated the file. Replaces per-row <code>comment[tool metadata]</code> column</td>
<td>lesSDRF v0.1.0, PRIDE</td>
<td><span class="optional-badge">OPTIONAL</span></td>
...
|`template` |Template declaration with version. Use one `#template` line per template. Format: `template_name,version=vX.Y.Z` |human,version=v1.1.0 |OPTIONAL |PRIDE:0000832
|`source` |Tool or origin that generated the file. Replaces per-row `comment[tool metadata]` column |lesSDRF v0.1.0 |OPTIONAL |NCIT:C25683
...
==== Template Inheritance and Versioning Rules

**Template format:** Each template is declared on its own line with its version:

[source,tsv]
----
#template=template_name,version=vX.Y.Z
----

**Template inheritance:** Only list leaf templates. Parent templates are implied by inheritance:

[source,tsv]
----
# crosslinking inherits from ms-proteomics (no need to list ms-proteomics)
#template=human,version=v1.1.0
#template=crosslinking,version=v1.0.0

# olink inherits from affinity-proteomics (no need to list affinity-proteomics)
#template=human,version=v1.1.0
#template=olink,version=v1.0.0
----

**Independent versioning:** Each template can have its own version, allowing templates to evolve independently:

[source,tsv]
----
# Templates at different versions
#template=human,version=v1.2.0
#template=crosslinking,version=v1.0.0
#template=phosphoproteomics,version=v1.1.0
----

**Source field:** The `#source` header replaces the per-row `comment[tool metadata]` column. Use it to record the tool or origin that generated the SDRF file (e.g., `lesSDRF v0.1.0`, `PRIDE`, `user-generated`).

NOTE: Header comments are OPTIONAL. SDRF files without header comments are still valid. When present, header comments provide valuable provenance information and enable tools to handle version-specific features appropriately. Header property names use underscores (e.g., `file_format`) rather than spaces to maintain consistency with the tab-delimited nature of SDRF files and avoid ambiguity when parsing.
...
<td class="sample-col">SN=sample_1;SN=sample_2</td>
<td class="sample-col">homo sapiens</td>
...
<td class="data-col">NT=Carbamidomethyl;AC=UNIMOD:4;TA=C;MT=fixed;PP=Anywhere</td>
<td class="data-col">NT=Oxidation;AC=UNIMOD:35;TA=M;MT=variable;PP=Anywhere</td>
...
|**BASE**
|Internal construction artifact with shared columns (not user-facing)
|_base_ (internal only)
...
|link:templates/cell-lines/README.adoc[cell-lines], link:templates/single-cell/README.adoc[single-cell], link:templates/crosslinking/README.adoc[crosslinking], link:templates/dda-acquisition/README.adoc[dda-acquisition], link:templates/dia-acquisition/README.adoc[dia-acquisition]
|No - must combine with TECHNOLOGY
|===

TIP: The **base** template is an internal construction artifact that should never be used directly by users. Start with a **technology template** instead: link:templates/ms-proteomics/README.adoc[ms-proteomics] for mass spectrometry experiments or link:templates/affinity-proteomics/README.adoc[affinity-proteomics] for Olink/SomaScan experiments. All columns defined in the <<from-sample-metadata,sample metadata>> and <<from-sample-data,data file metadata>> sections are inherited by these technology templates.
...
NOTE: When a column is marked as RECOMMENDED or OPTIONAL, it means users should evaluate whether the column applies to their specific experiment. For example, `characteristics[cell line]` is RECOMMENDED in the immunopeptidomics template because cell lines are commonly used, but tissue samples may use `not applicable`.

TIP: **For developers and maintainers:** For detailed information on YAML template structure, validators, and LinkML schema, see the link:metadata-guidelines/template-definitions.adoc[Template Definitions Guidelines].

=== Specifying Templates in File Headers
...
When combining templates from different branches of the hierarchy (e.g., organism + experiment), list each on its own line with its version:
...
**Rule 3: Independent versioning**

Each template is declared on its own line with its version, allowing templates to evolve independently:
...
|Human MS proteomics
|human (inherits ms-proteomics)
|`#template=human,version=v1.1.0`

|Mouse MS proteomics
|vertebrates (inherits ms-proteomics)
|`#template=vertebrates,version=v1.1.0`

|Non-organism-specific MS
|ms-proteomics only
|`#template=ms-proteomics,version=v1.1.0`

|Human crosslinking
|human + crosslinking
|`#template=human,version=v1.1.0` + `#template=crosslinking,version=v1.0.0`

|Recombinant protein crosslinking
|crosslinking only (no organism template)
|`#template=crosslinking,version=v1.0.0`

|Human Olink
|human + olink
|`#template=human,version=v1.1.0` + `#template=olink,version=v1.0.0`

|Human DIA with cell lines
|human + dia-acquisition + cell-lines
|`#template=human,version=v1.1.0` + `#template=dia-acquisition,version=v1.0.0` + `#template=cell-lines,version=v1.0.0`
|===
...
| **link:templates/ms-proteomics/README.adoc[MS-Proteomics]**
| All mass spectrometry-based proteomics (DDA, DIA, SRM/MRM/PRM)
| link:templates/ms-proteomics/README.adoc[Full documentation]
| **Yes** - minimum for MS

| **link:templates/affinity-proteomics/README.adoc[Affinity-Proteomics]**
| Affinity-based proteomics (Olink, SomaScan)
| link:templates/affinity-proteomics/README.adoc[Full documentation]
...
| **link:templates/human/README.adoc[Human]**
| Human clinical samples
| disease, age, sex, ancestry category, individual
| link:templates/human/README.adoc[Full documentation]

| **link:templates/vertebrates/README.adoc[Vertebrates]**
| Mouse, rat, zebrafish, other vertebrates
| disease, developmental stage, strain/breed, sex
| link:templates/vertebrates/README.adoc[Full documentation]

| **link:templates/invertebrates/README.adoc[Invertebrates]**
| Drosophila, C. elegans, insects
| disease, developmental stage, strain/breed, genotype
| link:templates/invertebrates/README.adoc[Full documentation]

| **link:templates/plants/README.adoc[Plants]**
| Arabidopsis, crops, other plants
| disease, developmental stage, strain/breed, growth conditions, treatment
| link:templates/plants/README.adoc[Full documentation]
|===
...
** Column names (text inside brackets) MUST be valid ontology terms - free text is not allowed
** Use specific ontology terms (e.g., `characteristics[tumor grading]` from NCIT, not `characteristics[grade]`)
** Search the https://www.ebi.ac.uk/ols4/[EBI Ontology Lookup Service (OLS)] to find appropriate ontology terms for your column names

3. **Use controlled vocabularies for cell values:**
** Reference existing ontologies (EFO, MONDO, UBERON, etc.) for cell values when available
** For cell values, ontology terms are recommended but free text is allowed when the exact term exists in EFO
...
* link:metadata-guidelines/sample-metadata.html#common-additional-columns[Common Additional Columns] in the Sample Metadata Guidelines
* link:sdrf-terms.html[SDRF Terms Reference] for a comprehensive list of commonly used terms
...
TIP: When adding custom columns, you MUST use valid ontology terms for column names. Check the link:metadata-guidelines/sdrf-terms.tsv[SDRF Terms Reference] and https://www.ebi.ac.uk/ols4/[EBI Ontology Lookup Service (OLS)] to find existing ontology terms that match your metadata needs. Free text column names are not allowed - each column name must be backed by an ontology term.
...
1. Open an https://github.com/bigbio/proteomics-metadata-standard/issues[issue on GitHub] describing the experiment type
