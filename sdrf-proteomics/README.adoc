...
This pattern preserves the **one row per MS run** rule (see <<row-uniqueness>>) - duplicating rows to describe the same acquisition is NOT permitted. Pipelines and converters (e.g. https://github.com/bigbio/quantms[quantms], https://github.com/bigbio/quantmsdiann[quantmsdiann]) read `comment[file uri]` and every `comment[associated file uri]` together so that the primary and auxiliary files are downloaded as a unit before conversion.

TIP: Once repositories standardise on bundling related files into a single `.zip` or `.tar.gz` (as already done for some Bruker timsTOF deposits), `comment[data file]` and `comment[file uri]` can reference the archive directly and `comment[associated data file]` is no longer needed.
