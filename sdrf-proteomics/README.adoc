...
Further detailed information, including any updates to this document, implementations, and examples is available at https://github.com/bigbio/proteomics-metadata-standard[SDRF GitHub Repository]. The official PSI web page for the document is the following: http://psidev.info/sdrf.
...
|===
|source name |characteristics [organism] |characteristics [disease] |assay name |comment [label] |comment [instrument] |comment [cleavage agent details] |comment [data file]

|sample_1 |homo sapiens |normal |run_1 |label free sample |Q Exactive HF |NT=Trypsin;AC=MS:1001251 |sample_1.raw
|sample_2 |homo sapiens |hepatocellular carcinoma |run_2 |label free sample |Q Exactive HF |NT=Trypsin;AC=MS:1001251 |sample_2.raw
|===
_Blue columns: Sample metadata (characteristics) | Green columns: Data file metadata (comments)_
endif::[]
...
* https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/core-templates[Core templates]: Organism-based templates (human, vertebrates, plants, etc.) that define base schemas for common proteomics experiments. See the <<core-templates,Core Templates>> section.

* https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/templates[Specialized templates]: Complete schemas for specific experiment types and acquisition methods (DDA acquisition, DIA acquisition, cell-lines, single-cell, affinity-proteomics, crosslinking, immunopeptidomics, metaproteomics). Each template has its own directory containing:
- A detailed README.adoc with checklists, and examples.
- A template file ({name}-template.sdrf.tsv) with column headers.

* https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/metadata-guidelines[Annotation guidelines]: Detailed documentation for specific metadata annotations (e.g., patient pre-existing condition, sample metadata, data file metadata).
...
<td class="data-col">Q Exactive HF</td>
<td class="data-col">sample_2.raw</td>
<td class="factor-col">liver cancer</td>
</tr>
</tbody>
</table>
<div class="sdrf-legend">
...
Example of an SDRF file with header comments (simplified example showing only select columns; see <<core-templates>> for complete required columns):
...
4+^s|*Disease* (see link:metadata-guidelines/sample-metadata.adoc#disease-annotation[Disease Annotation Guidelines])
...
- *characteristics[disease]*: The disease under study in the Sample. For healthy/control samples, use `normal` (see link:metadata-guidelines/sample-metadata.adoc#disease-annotation[Disease Annotation Guidelines]).
...
- Each characteristic name in the column header SHOULD be a CV term from the EFO ontology. For example, the header _characteristics[organism]_ corresponds to the ontology term Organism. However the values could be from EFO or other ontologies. For example, we RECOMMEND to use MONDO for diseases because it has better coverage than EFO. For healthy samples, use `normal` (PATO:0000461) - see link:metadata-guidelines/sample-metadata.adoc#disease-annotation[Disease Annotation Guidelines].
...
- **characteristics[biological replicate]**: Identifies independent biological samples (e.g., different patients, different cell cultures)
...
TIP: For pooled reference samples (e.g., TMT reference channels), use `pooled` for individual-specific fields including **biological replicate**, age, sex, and individual. This clearly indicates that the value represents a mixture rather than a single sample. If all pooled samples share a value (e.g., all females, or age range 40Y-50Y), that shared value MAY be used instead.
...
<thead>
<tr>
<th class="sample-col">characteristics [mass]</th>
<th class="sample-col">characteristics [spiked compound]</th>
</tr>
</thead>
<tbody>
<tr>
<td class="sample-col">1 ug</td>
<td class="sample-col">CT=peptide;PS=PEPTIDESEQ;QY=10 fmol</td>
</tr>
</tbody>
</table>
<div class="sdrf-legend">
...
* link:metadata-guidelines/sample-metadata.adoc[Sample Metadata Guidelines] - Detailed guidelines for age, sex, disease, organism part, cell type, developmental stage, and other sample characteristics
* link:metadata-guidelines/human-sample-metadata.adoc[Human Sample Metadata Guidelines] - Human-specific metadata including disease staging, treatment history, demographics, and lifestyle factors
...
|REQUIRED
|Enzyme information (use "not available" for undigested samples)
...
TIP: For DIA experiments, additional properties like MS1 scan range can be captured. See link:metadata-guidelines/data-file-metadata.adoc#dia-scan-window[DIA Scan Window Limits] in the Data File Metadata Guidelines.

=== Data File Metadata Guidelines

For detailed guidance on data file metadata, refer to the conventions document:

* link:metadata-guidelines/data-file-metadata.adoc[Data File Metadata Guidelines] - Detailed guidelines for labels, instruments, modifications, cleavage agents, mass tolerances, RAW file URIs, and other data file properties
...
- **Acquisition templates** (DDA, DIA) define acquisition-method-specific requirements
- **Experiment-type templates** (single-cell, immunopeptidomics, crosslinking, etc.) define methodology-specific requirements
...
- `allow_not_applicable` / `allow_not_available`: Whether special values are permitted
- `validators`: Validation rules (ontology checks, patterns, value constraints)

Template files are located in the https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/core-templates[core-templates] and https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/templates[templates] directories.
...
Then, if applicable, also apply an <<experiment-templates,experiment-type template>> for specialized methodologies.
...
| link:metadata-guidelines/sample-metadata.adoc#organism-annotation[characteristics[organism]] | Sample | Required | homo sapiens
| link:metadata-guidelines/sample-metadata.adoc#organism-part-annotation[characteristics[organism part]] | Sample | Required | liver
| link:metadata-guidelines/sample-metadata.adoc#disease-annotation[characteristics[disease]] | Sample | Required | normal
| characteristics[biological replicate] | Sample | Required | 1

4+^| *Data File Metadata*
| link:metadata-guidelines/data-file-metadata.adoc#assay-name[assay name] | Data | Required | run_1
| link:metadata-guidelines/data-file-metadata.adoc#technology-type[technology type] | Data | Required | proteomic profiling by mass spectrometry
| link:metadata-guidelines/data-file-metadata.adoc#data-acquisition-method[comment[proteomics data acquisition method]] | Data | Required | data-dependent acquisition
| link:metadata-guidelines/data-file-metadata.adoc#label-annotations[comment[label]] | Data | Required | label free sample
| link:metadata-guidelines/data-file-metadata.adoc#instrument[comment[instrument]] | Data | Required | Q Exactive HF
| link:metadata-guidelines/data-file-metadata.adoc#cleavage-agents[comment[cleavage agent details]] | Data | Required | NT=Trypsin;AC=MS:1001251
| link:metadata-guidelines/data-file-metadata.adoc#fraction-identifier[comment[fraction identifier]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#technical-replicate[comment[technical replicate]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#data-file[comment[data file]] | Data | Required | sample_1.raw
|===

Template file: https://github.com/bigbio/proteomics-metadata-standard/blob/master/sdrf-proteomics/core-templates/sdrf-default.sdrf.tsv[sdrf-default.sdrf.tsv]

NOTE: The link:metadata-guidelines/sample-metadata.adoc#cell-type-annotation[characteristics[cell type]] column is RECOMMENDED across all templates when the cell type is known or can be determined. Use `not available` if the cell type cannot be determined (e.g., whole tissue samples, mixed cell populations). For cell line experiments, use the cell-lines template which provides more specific guidance.
...
| link:metadata-guidelines/sample-metadata.adoc#organism-annotation[characteristics[organism]] | Sample | Required | homo sapiens
| link:metadata-guidelines/sample-metadata.adoc#organism-part-annotation[characteristics[organism part]] | Sample | Required | liver
| link:metadata-guidelines/sample-metadata.adoc#disease-annotation[characteristics[disease]] | Sample | Required | hepatocellular carcinoma
| link:metadata-guidelines/sample-metadata.adoc#cell-type-annotation[characteristics[cell type]] | Sample | Recommended | hepatocyte
| characteristics[biological replicate] | Sample | Required | 1
| link:metadata-guidelines/sample-metadata.adoc#age-annotation[characteristics[age]] | Sample | Required | 45Y
| link:metadata-guidelines/sample-metadata.adoc#sex-annotation[characteristics[sex]] | Sample | Required | male
| link:metadata-guidelines/human-sample-metadata.adoc#clinical-metadata[characteristics[ancestry category]] | Sample | Recommended | European
| link:metadata-guidelines/human-sample-metadata.adoc#individual-annotation[characteristics[individual]] | Sample | Recommended | P001

4+^| *Data File Metadata*
| link:metadata-guidelines/data-file-metadata.adoc#assay-name[assay name] | Data | Required | patient_001_run1
| link:metadata-guidelines/data-file-metadata.adoc#technology-type[technology type] | Data | Required | proteomic profiling by mass spectrometry
| link:metadata-guidelines/data-file-metadata.adoc#data-acquisition-method[comment[proteomics data acquisition method]] | Data | Required | data-dependent acquisition
| link:metadata-guidelines/data-file-metadata.adoc#label-annotations[comment[label]] | Data | Required | label free sample
| link:metadata-guidelines/data-file-metadata.adoc#instrument[comment[instrument]] | Data | Required | Orbitrap Exploris 480
| link:metadata-guidelines/data-file-metadata.adoc#cleavage-agents[comment[cleavage agent details]] | Data | Required | NT=Trypsin;AC=MS:1001251
| link:metadata-guidelines/data-file-metadata.adoc#fraction-identifier[comment[fraction identifier]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#technical-replicate[comment[technical replicate]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#data-file[comment[data file]] | Data | Required | patient_001.raw
|===

Template file: https://github.com/bigbio/proteomics-metadata-standard/blob/master/sdrf-proteomics/core-templates/sdrf-human.sdrf.tsv[sdrf-human.sdrf.tsv]

NOTE: The characteristics[individual] column is optional and is only used when a code for the individual is available; if not, use **not available**. For age encoding format, see link:metadata-guidelines/sample-metadata.adoc[Sample Metadata Guidelines].
...
| link:metadata-guidelines/sample-metadata.adoc#organism-annotation[characteristics[organism]] | Sample | Required | mus musculus
| link:metadata-guidelines/sample-metadata.adoc#organism-part-annotation[characteristics[organism part]] | Sample | Required | brain
| link:metadata-guidelines/sample-metadata.adoc#disease-annotation[characteristics[disease]] | Sample | Required | normal
| link:metadata-guidelines/sample-metadata.adoc#cell-type-annotation[characteristics[cell type]] | Sample | Recommended | neuron
| characteristics[biological replicate] | Sample | Required | 1
| link:metadata-guidelines/sample-metadata.adoc#developmental-stage[characteristics[developmental stage]] | Sample | Recommended | adult

4+^| *Data File Metadata*
| link:metadata-guidelines/data-file-metadata.adoc#assay-name[assay name] | Data | Required | mouse_001_run1
| link:metadata-guidelines/data-file-metadata.adoc#technology-type[technology type] | Data | Required | proteomic profiling by mass spectrometry
| link:metadata-guidelines/data-file-metadata.adoc#data-acquisition-method[comment[proteomics data acquisition method]] | Data | Required | data-dependent acquisition
| link:metadata-guidelines/data-file-metadata.adoc#label-annotations[comment[label]] | Data | Required | label free sample
| link:metadata-guidelines/data-file-metadata.adoc#instrument[comment[instrument]] | Data | Required | timsTOF Pro
| link:metadata-guidelines/data-file-metadata.adoc#cleavage-agents[comment[cleavage agent details]] | Data | Required | NT=Trypsin;AC=MS:1001251
| link:metadata-guidelines/data-file-metadata.adoc#fraction-identifier[comment[fraction identifier]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#technical-replicate[comment[technical replicate]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#data-file[comment[data file]] | Data | Required | mouse_001.raw
|===

Template file: https://github.com/bigbio/proteomics-metadata-standard/blob/master/sdrf-proteomics/core-templates/sdrf-vertebrates.sdrf.tsv[sdrf-vertebrates.sdrf.tsv]
...
| link:metadata-guidelines/sample-metadata.adoc#organism-annotation[characteristics[organism]] | Sample | Required | drosophila melanogaster
| link:metadata-guidelines/sample-metadata.adoc#organism-part-annotation[characteristics[organism part]] | Sample | Required | head
| link:metadata-guidelines/sample-metadata.adoc#disease-annotation[characteristics[disease]] | Sample | Required | normal
| link:metadata-guidelines/sample-metadata.adoc#cell-type-annotation[characteristics[cell type]] | Sample | Recommended | neuron
| characteristics[biological replicate] | Sample | Required | 1

4+^| *Data File Metadata*
| link:metadata-guidelines/data-file-metadata.adoc#assay-name[assay name] | Data | Required | fly_sample_1_run1
| link:metadata-guidelines/data-file-metadata.adoc#technology-type[technology type] | Data | Required | proteomic profiling by mass spectrometry
| link:metadata-guidelines/data-file-metadata.adoc#data-acquisition-method[comment[proteomics data acquisition method]] | Data | Required | data-dependent acquisition
| link:metadata-guidelines/data-file-metadata.adoc#label-annotations[comment[label]] | Data | Required | label free sample
| link:metadata-guidelines/data-file-metadata.adoc#instrument[comment[instrument]] | Data | Required | Q Exactive Plus
| link:metadata-guidelines/data-file-metadata.adoc#cleavage-agents[comment[cleavage agent details]] | Data | Required | NT=Trypsin;AC=MS:1001251
| link:metadata-guidelines/data-file-metadata.adoc#fraction-identifier[comment[fraction identifier]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#technical-replicate[comment[technical replicate]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#data-file[comment[data file]] | Data | Required | fly_sample_1.raw
|===

Template file: https://github.com/bigbio/proteomics-metadata-standard/blob/master/sdrf-proteomics/core-templates/sdrf-invertebrates.sdrf.tsv[sdrf-invertebrates.sdrf.tsv]
...
| link:metadata-guidelines/sample-metadata.adoc#organism-annotation[characteristics[organism]] | Sample | Required | arabidopsis thaliana
| link:metadata-guidelines/sample-metadata.adoc#organism-part-annotation[characteristics[organism part]] | Sample | Required | leaf
| link:metadata-guidelines/sample-metadata.adoc#disease-annotation[characteristics[disease]] | Sample | Required | normal
| link:metadata-guidelines/sample-metadata.adoc#cell-type-annotation[characteristics[cell type]] | Sample | Recommended | guard cell
| characteristics[biological replicate] | Sample | Required | 1

4+^| *Data File Metadata*
| link:metadata-guidelines/data-file-metadata.adoc#assay-name[assay name] | Data | Required | arabidopsis_col0_run1
| link:metadata-guidelines/data-file-metadata.adoc#technology-type[technology type] | Data | Required | proteomic profiling by mass spectrometry
| link:metadata-guidelines/data-file-metadata.adoc#data-acquisition-method[comment[proteomics data acquisition method]] | Data | Required | data-dependent acquisition
| link:metadata-guidelines/data-file-metadata.adoc#label-annotations[comment[label]] | Data | Required | label free sample
| link:metadata-guidelines/data-file-metadata.adoc#instrument[comment[instrument]] | Data | Required | Orbitrap Fusion Lumos
| link:metadata-guidelines/data-file-metadata.adoc#cleavage-agents[comment[cleavage agent details]] | Data | Required | NT=Trypsin;AC=MS:1001251
| link:metadata-guidelines/data-file-metadata.adoc#fraction-identifier[comment[fraction identifier]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#technical-replicate[comment[technical replicate]] | Data | Required | 1
| link:metadata-guidelines/data-file-metadata.adoc#data-file[comment[data file]] | Data | Required | arabidopsis_col0.raw
|===

Template file: https://github.com/bigbio/proteomics-metadata-standard/blob/master/sdrf-proteomics/core-templates/sdrf-plants.sdrf.tsv[sdrf-plants.sdrf.tsv]
...
In addition to core organism-based templates, SDRF-Proteomics provides specialized templates for specific acquisition methods and experiment types. These templates extend the core templates with methodology-specific columns.

==== Acquisition Templates

Templates for specific mass spectrometry acquisition methods:

- **link:templates/dda-acquisition/README.adoc[DDA Acquisition]**: Data-dependent acquisition experiments. Includes columns for dissociation method, collision energy, fractionation method, modification parameters, and mass tolerances.
- **link:templates/dia-acquisition/README.adoc[DIA Acquisition]**: Data-independent acquisition experiments. Includes columns for scan window limits, isolation window width, DIA method, and spectral library information.

==== Methodology Templates

Templates for specialized proteomics methodologies (under development):

- **link:templates/cell-lines/README.adoc[Cell Lines]**: Experiments using cell line samples. Includes Cellosaurus integration for cell line identification.
- **link:templates/single-cell/README.adoc[Single-Cell Proteomics]**: Single-cell proteomics experiments. Includes columns for cell isolation method, carrier proteome, and single-cell identifiers.
- **link:templates/immunopeptidomics/README.adoc[Immunopeptidomics]**: MHC peptide immunopeptidomics. Includes columns for MHC class, HLA typing, and enrichment methods.
- **link:templates/crosslinking/README.adoc[Crosslinking MS]**: Cross-linking mass spectrometry experiments. Includes columns for crosslinking reagents and enrichment methods.
- **link:templates/metaproteomics/README.adoc[Metaproteomics]**: Environmental and microbiome proteomics. Includes columns for environmental sample type and geographic location.
- **link:templates/affinity-proteomics/README.adoc[Affinity Proteomics]**: Affinity-based proteomics (Olink, SomaScan). Includes columns specific to these platforms.
...
<td>...</td>
<td class="sample-col">heart</td>
<td class="sample-col">heart left ventricle</td>
<td>...</td>
</tr>
</tbody>
</table>
...
The template files can be downloaded from the https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/core-templates[core-templates] folder.
...
NOTE: Use `normal` (not "control") in the disease field for healthy samples. "Control" is an experimental design concept, not a disease state. See link:metadata-guidelines/sample-metadata.adoc#disease-annotation[Disease Annotation Guidelines] for details.
...
<th>...</th>
<th class="sample-col">characteristics [disease]</th>
<th class="sample-col">characteristics [disease stage]</th>
<th>...</th>
<th class="factor-col">factor value [disease]</th>
<th class="factor-col">factor value [disease stage]</th>
</tr>
</thead>
<tbody>
<tr>
...
For detailed specifications and examples, visit the https://github.com/bigbio/proteomics-metadata-standard/tree/master/sdrf-proteomics/templates[Experiment-specific Templates] documentation.
