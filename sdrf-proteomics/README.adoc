...
|===
|source name |characteristics[organism] |characteristics[organism part] |characteristics[disease] |characteristics[biological replicate]

|sample_1 |homo sapiens |liver |normal |1
|sample_2 |homo sapiens |liver |hepatocellular carcinoma |1
|===
...
|===
|source name |assay name |technology type |comment[data acquisition method] |comment[label] |comment[instrument] |comment[cleavage agent details] |comment[fraction identifier] |comment[data file]

|sample_1 |run_1 |proteomic profiling by mass spectrometry |data-dependent acquisition |label free sample |Q Exactive HF |NT=Trypsin;AC=MS:1001251 |1 |sample_1.raw
|sample_2 |run_2 |proteomic profiling by mass spectrometry |data-dependent acquisition |label free sample |Q Exactive HF |NT=Trypsin;AC=MS:1001251 |1 |sample_2.raw
|===

TIP: In a real SDRF file, all columns appear in a single row. The tables above are split for readability.
