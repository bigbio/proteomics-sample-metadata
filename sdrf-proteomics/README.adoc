...
To integrate with a BioSamples database, use the _characteristics[biosample accession number]_ column to specify the BioSamples accession identifier for each sample. This characteristic provides a direct link between the proteomics sample metadata and the corresponding BioSamples database entry.
...
|sample_003     | mus musculus              | SAMEA12345680                                | brain                          | normal
|===
...
NOTE: BioSample accession numbers from NCBI follow the format SAMN############ and from EBI follow the format SAMEA##########, where # represents digits. Either NCBI or EBI BioSample accession numbers can be used depending on where the sample is registered. When available, providing BioSample accession numbers is RECOMMENDED to enhance data integration and reusability.
