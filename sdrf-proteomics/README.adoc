...
|`comment[sdrf template]` |Template name and version used for annotation. Format: `NT=template_name;version=vX.Y.Z`. Multiple templates can be specified using multiple columns with the same name. |NT=human;version=v1.1.0 |OPTIONAL |PRIDE:0000832
|`comment[sdrf annotation tool]` |Software tool, script, or method used to generate or annotate the SDRF file. Format: `NT=tool_name;version=vX.Y.Z` |NT=lesSDRF;version=v0.1.0 |OPTIONAL |PRIDE:0000840
