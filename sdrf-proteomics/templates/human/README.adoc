...
All columns from the link:../base/README.adoc[base template] are inherited (source name, organism, organism part, biological replicate, assay name, technology type, instrument, technical replicate, data file). Technology-specific columns (cleavage agent, label, fraction identifier, etc.) come from the technology template (`ms-proteomics` or `affinity-proteomics`).
