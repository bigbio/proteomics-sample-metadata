...
All columns from the link:../base/README.adoc[base template] are inherited (source name, organism, organism part, biological replicate, assay name, technology type, instrument, technical replicate, data file). Technology-specific columns (cleavage agent, label, fraction identifier, etc.) come from the technology template (`ms-base` or `affinity-proteomics`).
...
For general phenotype annotation guidelines applicable to all organisms, see link:../../metadata-guidelines/sample-metadata.adoc#phenotype-annotation[Sample Metadata Guidelines - Phenotype].
