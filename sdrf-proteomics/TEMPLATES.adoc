...
| dia-acquisition, single-cell, lc-ms-metabolomics, gc-ms-metabolomics, cell-lines
|===
...
1. **ONE technology template** (required): `ms-proteomics`, `affinity-proteomics`, or `ms-metabolomics`
...
3. **ZERO or more experiment templates**: `dia-acquisition`, `single-cell`, `crosslinking`, `immunopeptidomics` (extend `ms-proteomics`), `lc-ms-metabolomics`, `gc-ms-metabolomics` (extend `ms-metabolomics`)
...
* **Technology**: `ms-proteomics`, `affinity-proteomics`, and `ms-metabolomics` are mutually exclusive
...
* **Metabolomics modality**: `lc-ms-metabolomics` and `gc-ms-metabolomics` cannot be combined for the same assay
* **Metabolomics experiments**: `lc-ms-metabolomics`/`gc-ms-metabolomics` require `ms-metabolomics` (not `ms-proteomics`)
...
| Human affinity proteomics study | `affinity-proteomics` + `human`
| Human LC-MS metabolomics | `lc-ms-metabolomics` + `human`
| Plant GC-MS metabolomics | `gc-ms-metabolomics` + `plants`
...
| Affinity-based proteomics (Olink, SomaScan) | `affinity-proteomics`
| LC-MS metabolomics | `lc-ms-metabolomics` (inherits `ms-metabolomics`)
| GC-MS metabolomics | `gc-ms-metabolomics` (inherits `ms-metabolomics`)
|===
