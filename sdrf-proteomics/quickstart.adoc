...
- Each **row** = one MS run (one raw file)
...
[NOTE]
====
**One row = one raw file.** If you have 10 raw files, you'll have 10 rows. If the same sample was run multiple times (technical replicates), create one row per run with the same source name but different data files.
...
Here's a complete SDRF file for a human liver cancer study comparing tumor vs. healthy tissue:
...
- 4 samples from human liver tissue
- 2 cancer patients, 2 healthy controls
